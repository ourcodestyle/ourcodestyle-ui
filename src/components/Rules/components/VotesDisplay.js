...
const VotesDisplay = ({ votes, vertical, withIntent, hasConsensus }) => {
  ...
  if (withIntent) {
    const buildUserIconForVote = (vote) => <UserIconWithName key={vote.user.id} user={vote.user} />
    iconsHoverList = <div style={{ padding: 10, display: 'grid', gridTemplateColumns: '200px 200px' }}>
      <div>
        <p><Icon icon={IconNames.THUMBS_UP} /></p>
        {_.filter(votes, { intent: 'positive' }).map(buildUserIconForVote)}
      </div>
      <div>
        <p><Icon icon={IconNames.THUMBS_DOWN} /></p>
        {_.filter(votes, { intent: 'negative' }).map(buildUserIconForVote)}
      </div>
    </div>
  } else {
...
VotesDisplay.propTypes = {
  votes: PropTypes.array.isRequired,
  vertical: PropTypes.bool,
  withIntent: PropTypes.bool,
